...
#SBATCH --output="results/output/hybrid_finite.out"
...
@everywhere params = (; L=2000.0, α_SM=40.0, a0=10.0, U_SC=-30.0, μ_SC=20.0, τ=0.8) # nm & meV (ħ²/mₑ)

@everywhere μs_SM = range(-8.8, -7.0, 144) #mev (ħ²/mₑ)
...
begin
    fixedpoint_scanner = (Vz, μ_SM) -> fixedpoint(Sf, GS.Spectrum(); params..., m=1, beta=0.8, Vz, μ_SM)
    @time seis = selfconsistency(fixedpoint_scanner, Vzs, μs_SM)
